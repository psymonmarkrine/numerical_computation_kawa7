...
\begin{align}
g_1 = & \lambda - a_{11} \cr
g_k = & (\lambda - a_{kk}) - \frac{a_{k\ k-1}^2}{g_{k-1}} & (2 \leqq k \leqq n) \tag{4.18} \cr
\end{align}
$$
...
任意の $n$ 次複素正方行列 $A$ は、あるユニタリ行列 $Q$ によって複素上三角行列 $R$ に変換出来る (シュ－ルの定理<sup>[$18)$](#note18)</sup> )。
これが、これから述べる $\text{QR}$ 法とダブル $\text{QR}$ 法の出発点である。

> <small id="note18">$^{18)}$ シュ－ルの定理は 問題 4-1 で証明されている。</small>
...
シュ－ルの定理 $(4.147)$ は、任意の複素非対称行列 $A$ について成り立つ。

...
(1) & x_0 を与える \cr
 & \quad r_0 = b - A x_0 \cr
(2) & k = 0, 1, 2, \cdots の順に \cr
 & \quad w_k = A r_k \cr
 & \quad \alpha_k = \displaystyle\frac{(r_k, r_k)}{(r_k , w_k)} \cr
 & \quad x_{k+1} = x_k + \alpha_k r_k \cr
 & \quad r_{k+1} = r_k - \alpha_k w_k \cr
\end{array}
$$
...
(2) \quad k = 0, 1, 2, \cdots の順に収束判定条件 (c_1 \leqq c_2) が満たされるまで反復 \cr
\qquad \quad c_1 = c_2 \cr
\qquad \quad q_k = A p_k \cr
...
\text{次数}(n) & \text{最大固有値} (\lambda_\text{max}) & \text{最小固有値} (\lambda_\text{min}) 条&\text{ 件数}(\text{cond}) \cr
1 & 1.0 & 1.0 & 1.0 \cr
2 & 1.0 & 1.0 & 1.0 \cr
3 & 1.0 & 1.0 & 1.0 \cr
4 & 1.0 & 1.0 & 1.0 \cr
...
$$
\alpha_k = \frac{(r_k,\ q_k)}{(q_k,\ q_k)} \quad ただし \quad q_k = A p_k
$$
